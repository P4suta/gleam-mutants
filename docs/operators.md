...
| `option-neutral` | `Some(x)` to `None`, qualifier kept |
...
`Result` that is built is left alone rather than mutated on a hunch. An
arbitrary expression is not treated as an integer, float, string, list, or
option by guesswork. Candidates that are emitted but fail compiler
